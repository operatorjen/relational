...
export class Relational {
  constructor(agents = [], config = {}) {
    ...
    if (this.config.analysisInterval > 0) this.analysisTimer = setInterval(() => { this._performPeriodicAnalysis() }, this.config.analysisInterval)
  }
  ...
  _performPeriodicAnalysis() {
    const dy = this.getSocialDynamics(), dg = this.getDiagnostics()
    this._emit('analytics', {
      dynamics: dy,
      diagnostics: dg,
      timestamp: Date.now()
    })
    if (dy.networkHealth < 0.3) {
      this._emit('healthWarning', {
        level: 'warning',
        message: 'Low network health detected',
        health: dy.networkHealth
      })
    }
  }
  ...
  _emit(event, data) { if (this.config.onEvent) this.config.onEvent(event, data) }
}
